...
class CanonicalESOptimizer(BaseOptimizer):
    ...
    def __init__(self, parameters, lam, rank, meta_pop_size, parent_id, u, settings):
        super().__init__(parameters, rank)
    ...
    def get_parameters(self):
        if self.rank < self.meta_pop_size:
            return None, self.parameters[self.parent_id]
        r_id = self.r_noise_id()
        p = self.parameters[self.parent_id] + self.sigma * self.noise_table[r_id:(r_id + self.n)]
        return r_id, p
    ...
    def update(self, ids, rewards):
        ...
        self.parameters[self.parent_id] += self.step
    ...
    def assign_weights(self, new_weights):
        self.parameters[self.parent_id] = self.parameters[new_weights]
    ...
    def log_path(self, game, network, run_name):
        return "logs_mpi/%s/Baseline/%s/%d/%f/%f/%f/%s" % \
               (game, network, self.lam, self.sigma, self.lr, self.c_sigma_factor, run_name)
